...
RETURNS TRIGGER AS $$
BEGIN
    IF (TG_OP = 'INSERT') AND (NEW.reaction = 'LIKED') THEN
        UPDATE videos
        SET likes_count = likes_count + 1
        WHERE  videos.id = NEW.video_id;

    ELSEIF (TG_OP = 'INSERT') AND (NEW.reaction = 'DISLIKED') THEN
        UPDATE videos
        SET dislikes_count = dislikes_count + 1
        WHERE  videos.id = NEW.video_id;  

    ELSEIF (TG_OP = 'DELETE') AND (OLD.reaction = 'DISLIKED') THEN
        UPDATE videos
        SET dislikes_count = dislikes_count - 1
        WHERE  videos.id = OLD.video_id;

    ELSEIF (TG_OP = 'DELETE') AND (OLD.reaction = 'LIKED') THEN
        UPDATE videos
        SET likes_count = likes_count - 1
        WHERE  videos.id = OLD.video_id; 

    ELSEIF (TG_OP = 'UPDATE') AND (OLD.reaction = 'LIKED') AND (NEW.reaction = 'DISLIKED') THEN
        UPDATE videos
        SET likes_count = likes_count - 1, dislikes_count = dislikes_count + 1
        WHERE  videos.id = OLD.video_id;

    ELSEIF (TG_OP = 'UPDATE') AND (OLD.reaction = 'DISLIKED') AND (NEW.reaction = 'LIKED') THEN
        UPDATE videos
        SET likes_count = likes_count + 1, dislikes_count = dislikes_count - 1
        WHERE  videos.id = OLD.video_id;
